---
title: "Putting the 'viz' back into 'riboviz': connecting Rmarkdown, Shiny and Nextflow"
...
<script>
    $(document).ready(function() {
      $('slide:not(.title-slide, .backdrop, .segue)').append('<footer label=\"bit.ly/EdinbR-reviz-riboviz"></footer>');    
    })
</script>
...
* Research Assistant in Bioinformatics at Wallace Lab, Institute for Cell Biology, University of Edinburgh
* developer on riboviz team  
* Edinburgh Carpentries instructor (foundational data & software carpentry coding courses)
* background mainly in R, have picked up other bits & bobs lately 
* this is my first project coding with others & using 'best practices' (code review, style guides, documentation!)
...
*Developed in collaborative project BBSRC-NSF funded project involving:* University of Edinburgh, EPCC, Rutgers University and University of California, Berkeley. 
...
Lots of Workflow Management System Benefits: 
...
# Adding Features to the Workflow  
...
1) Edit `Plot()` functions for more cohesion _(no data-wrangling!)_  
...
* {Shiny} package lets you build interactive web apps using R

* _Shiny code can't be run as part of a Nextflow workflow_ - kicks up a 'server' that prevents processes completing

* Adapting riboviz to generate a **'helper script'**:  
  - lets users generate the Shiny run command with _correct inputs & file paths_ added-in from Nextflow  

<font size=2>R shiny: https://shiny.rstudio.com/</font>
...
## helperViz Nextflow process

<div class="centered">
```{r, out.width = "800"}
knitr::include_graphics("https://raw.githubusercontent.com/FlicAnderson/EdinbR-20210519/main/images/nextflow-process_helperViz.png")
``` 
</div> 

<font size="2">
Line 1221-1223: builds 'script' from: Rscript command (to run R script in commandline), shiny R script & config file   
Line 1225: puts the script command (with correct paths) into new file 'interactive_viz.sh'  
</font>

<div class="notes">

Line 1214: starts the Nextflow process definition  

Line 1215: sets output location for files 

Line 1217: list input file (currently just config yaml)

Line 1219: tells Nextflow what the outputs of this process are (1x shell script file )  

Line 1221-1223: builds 'script' from: Rscript command (to run R script in commandline), shiny R script, config file 

Line 1225: actually executes the script (runs the echo command to put the script with correct parameters into new file 'interactive_viz.sh') 
...
Now we can:   
 * **run riboviz** as normal  
 * execute the **helper script**   
 * which runs the **Shiny code**   
 * ... to obtain **interactive visualization** goodness!  

We're still not quite finished the Shiny code - adding new plots, tweaking interactivity controls 

Users now have a hassle-free **no-editing-required** option for interactive plots based on riboviz analysis outputs

Lets them further **explore their data**, view specific samples / plots according to their interests
...
## Riboviz Shiny Output Example

<div class="centered">
```{r, out.width = "600"}
knitr::include_graphics("https://raw.githubusercontent.com/FlicAnderson/EdinbR-20210519/main/images/riboviz-output_shiny.png")
``` 
</div> 
...
**Shiny won't run in your Nextflow pipeline** but Nextflow can still help you with the file/parameter admin!
